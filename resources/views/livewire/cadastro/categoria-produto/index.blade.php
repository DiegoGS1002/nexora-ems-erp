<div class="nx-list-page">
    ...
    <div class="nx-page-header">
        <div class="nx-page-header-left">
            <h1 class="nx-page-title">Categorias de Produto</h1>
            <p class="nx-page-subtitle">Gerencie as categorias utilizadas no cadastro de produtos</p>
        </div>
        <div class="nx-page-actions">
            ...
            <a href="{{ route('product-categories.create') }}" class="nx-btn nx-btn-primary" wire:navigate>
    ...
    <div class="nx-card" style="padding:16px;">
        <div class="nx-field" style="max-width:360px;margin-bottom:16px;">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Nome da categoria...">
        </div>
            ...
            <table class="nx-table">
                ...
                <tbody>
                    ...
                    <tr wire:key="cat-{{ $cat->id }}">
                        <td>
                            <span style="display:inline-block;width:24px;height:24px;border-radius:6px;background:{{ $cat->color }};border:1px solid rgba(0,0,0,.1);"></span>
                        </td>
                        <td><strong>{{ $cat->name }}</strong></td>
                        <td>
                            <span style="font-family:monospace;font-size:12px;background:#F1F5F9;padding:2px 7px;border-radius:4px;color:#475569;">{{ $cat->slug }}</span>
                        </td>
                        <td>
                            @if($cat->description)
                                <span title="{{ $cat->description }}" style="display:block;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#64748B;font-size:13px;">{{ $cat->description }}</span>
                            @else
                                <span style="color:#CBD5E1;">—</span>
                            @endif
                        </td>
                        <td class="nx-td-center">
                            <span class="nx-badge {{ $cat->products_count > 0 ? 'nx-badge-success' : 'nx-badge-gray' }}">
                                {{ $cat->products_count }}
                            </span>
                        </td>
                        <td class="nx-td-center">
                        ...
                        <td class="nx-td-actions">
                            <a href="{{ route('product-categories.edit', $cat) }}" class="nx-action-btn nx-action-edit" title="Editar" wire:navigate>
                            ...
                            <button
                                ...
                                wire:confirm="Deseja excluir a categoria '{{ $cat->name }}'? Esta ação não pode ser desfeita."
                            >
        ...
        <div class="nx-table-footer">
            <span class="nx-table-count">{{ $categories->total() }} {{ $categories->total() === 1 ? 'categoria' : 'categorias' }}</span>
            <div class="nx-pagination">{{ $categories->links() }}</div>
        </div>
        @endif
    </div>
